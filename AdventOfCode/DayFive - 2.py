...
while pos < len(data):
    ...
    if oc.code == 1: # Add
        ...
        nPos = posFive(data, pos, oc)
        if nPos == pos:
            pos += 3
        else:
            pos = nPos
    elif oc.code == 6:
        nPos = posSix(data, pos, oc)
        if nPos == pos:
            pos += 3
        else:
            pos = nPos
    elif oc.code == 7:
        posSeven(data, pos, oc)
        pos += 4
    elif oc.code == 8:
        posEight(data, pos, oc)
        pos += 4
    elif oc.code == 99:
        break
    else:
        print("unrecognized opcode ", oc)